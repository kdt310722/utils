...
export type FlattenKeys<T, D extends string = '.'> = T extends AnyObject ? { [K in keyof T]: `${string & K}${FlattenKeys<T[K], D> extends never ? '' : `${D}${FlattenKeys<T[K], D>}`}` }[keyof T] : never
